...
async def setup(app, loop):
    ...
    app.ctx.pool = await aiomysql.create_pool(host="public-cbsv1.net.rikusutep.xyz", user=getenv("dms"), password=getenv("db_password"), loop=loop, db="b3vad_rtbot", autocommit=True)
    app.ctx.wslist = []
